  ...
  (function initTicketChecker() {
    ...
    function selectType(v) {
      const m = /^(INC|RITM|REQ|SCTASK|TASK)/.exec(v);
      if (!m) return;
...
// 🔥 REMOVER completamente setCustomValidity
if (!ok && v) {
  tn.dataset.invalid = "1";
} else {
  tn.dataset.invalid = "0";
}
  ...
  tn.setCustomValidity(""); // 🔥 LIMPA ERRO SEMPRE
...
const token = window.CATALOG_TOKEN || '';
  ...
  (function finalizeScript() {
    try {
      ...
      console.error("Erro ao finalizar scripts.js:", e);
              ...
              /* ===================== 
              Função para detectar tipo de ticket pelo número (INC/RITM/SCTASK) 
              ===================== */
  /*function detectTicketType(ticket) {
    const v = (ticket || "").toUpperCase().trim();

    if (v.startsWith("INC"))     return "INCIDENTE";
    if (v.startsWith("RITM"))    return "REQUISIÇÃO";
    if (v.startsWith("REQ"))     return "REQUISIÇÃO";
    if (v.startsWith("SCTASK"))  return "TASK";
  ...
  /* =========================================================
    MODO INTERATIVO — ETAPA 1 (Ticket) → ETAPA 2 (Fornecedor)
    ========================================================= */
  ...
  /* =========================================================
    ...
    USANDO TAB
    ========================================================= */
  (function initGuidedFlowTAB() {

    
 if (window.__guidedFlowInit) return;
  window.__guidedFlowInit = true;
    ...
    const stepTicket   = document.getElementById("guided-step-ticket");
    const stepSupplier = document.getElementById("guided-step-supplier");

    const ticketModalInput = document.getElementById("guided-ticket-input");
    const ticketTypeText   = document.getElementById("guided-ticket-type");
    const finalTypeText    = document.getElementById("guided-final-type");

    const supplierModalInput = document.getElementById("guided-supplier-input");

    const realTicketInput   = document.getElementById("ticket_number");
    const realSupplierInput = document.getElementById("audited_supplier_input");

    if (!startBtn || !ticketModalInput) return;

    function detectType(v) {
      v = (v || "").toUpperCase().trim();
      if (v.startsWith("INC")) return "INCIDENTE";
      if (v.startsWith("RITM") || v.startsWith("REQ")) return "REQUISIÇÃO";
      if (v.startsWith("SCTASK") || v.startsWith("TASK")) return "TASK";
      return "";
    }
    ...
    function openModal() {
      ...
      stepTicket.hidden = false;
      stepSupplier.hidden = true;

      ticketModalInput.value = "";
      ticketTypeText.textContent = "—";

      ticketModalInput.focus();
    }
    ...
    document.getElementById("guided-exit").addEventListener("click", closeModal);

    // 🔹 Atualiza tipo em tempo real
    ticketModalInput.addEventListener("input", () => {
      const type = detectType(ticketModalInput.value);
      ticketTypeText.textContent = type || "—";
    });

    // 🔹 TAB NO TICKET → AVANÇA
    ticketModalInput.addEventListener("keydown", (e) => {
      if (e.key !== "Tab") return;

      const value = ticketModalInput.value.trim();
      ...
      // Preenche o form real
      realTicketInput.value = value;
      realTicketInput.dispatchEvent(new Event("input", { bubbles: true }));

      finalTypeText.textContent = type;

      stepTicket.hidden = true;
      stepSupplier.hidden = false;

      e.preventDefault();
      supplierModalInput.focus();
    });

    // 🔹 TAB NO FORNECEDOR → VOLTA PRO FORM
    supplierModalInput.addEventListener("keydown", (e) => {
      if (e.key !== "Tab") return;
  ...
  /* =========================================================
    ASSISTENTE DE ANÁLISE DE TEXTO (Comunicação com Backend)
    ========================================================= */
...
/**
 * Coleta dados mínimos do formulário
 * (não envia tudo, só o necessário)
 */
function collectFormData() {
  return {
    audit_month: document.querySelector('[name="audit_month"]')?.value ?? null,
    location_id: document.querySelector('[name="location_id"]')?.value ?? null,
    item_id:      document.querySelector('[name="item_id"]')?.value ?? null,
    bdgc_quantity: document.querySelector('[name="bdgc_quantity"]')?.value ?? null,
    found_quantity: document.querySelector('[name="found_quantity"]')?.value ?? null,
    divergence_quantity: document.querySelector('[name="divergence_quantity"]')?.value ?? null
  };
}
  ...
  /* =========================================================
    FUNÇÃO DE EXPORTAÇÃO POR MÊS (Botão "Exportar XLSX")
    ========================================================= */
...
function exportByMonth() {
  ...
  // ✅ ROTA CORRETA, SEM PHP NO MEIO
  window.location.href =
    '/export/xlsx?audit_month=' + encodeURIComponent(value);
}

  
/* =========================================================
  CÁLCULO AUTOMÁTICO DE DIVERGÊNCIA ENTRE QUANTIDADE BDGC E ENCONTRADA
  ========================================================= */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Se não for a tela de estoque, não faz nada
  ...
  function updateDivergence() {
    ...
    if (diff === 0) {
      // ✅ OK
      display.classList.add('divergence-ok');
    } else {
      // ❌ Divergência
      display.classList.add('divergence-error');
    }
  }
