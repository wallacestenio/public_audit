...
window.location.href =
  base + '/export/xlsx?audit_month=' + encodeURIComponent(value);
  ...
  <div id="ticket-feedback" class="muted" style="display:none"></div>

  <!-- ✅ FICA AQUI -->
  <div id="ticket-type-info"
       class="muted"
       style="margin-top:6px">
    <!-- Tipo do ticket detectado: -->
    <strong id="ticket-type-text"><!-- — --></strong>
  </div>
...
>
<small class="muted">
  Em janeiro, auditorias de dezembro referem-se ao ano anterior.
</small>
...
/**
 * ============================
 * Assistente de Análise - Front
 * ============================
 */

const analyzeBtn = document.getElementById('assistant_analyze_btn');
const textArea   = document.getElementById('assistant_text');
const resultBox = document.querySelector('.assistant-response');
const resultContent = document.querySelector('.assistant-response-content');

/**
 * Habilita o botão apenas se houver texto suficiente
 */
textArea.addEventListener('input', () => {
  analyzeBtn.disabled = textArea.value.trim().length < 20;
});
...
analyzeBtn.addEventListener('click', () => {
  ...
  /* Chama a rota de análise, enviando o texto e alguns dados do formulário */
...
.then(data => {
  if (data.success) {
    ...
    renderError(data.message || 'Falha na análise');
  }
})
.catch(() => {
  renderError('Erro de comunicação com o assistente.');
});
...
function renderAnalysis(analysis) {
  ...
  if (analysis.points && analysis.points.length > 0) {
    ...
    html += '<p>Nenhum ponto relevante identificado.</p>';
  }

  html += `<p class="assistant-note">${analysis.note}</p>`;
...
(() => {
  ...
  const feedback = document.getElementById('ticket-feedback');
  if (!ticketInput || !feedback) return;
  ...
  function selectTicketType(typeValue) {
    document.querySelectorAll('input[name="ticket_type"]').forEach(radio => {
      ...
      if (radio.value === typeValue) {
        ...
        document.getElementById('ticket_type_hidden').value = typeValue;
      } else {
  ...
  function detectTypeAndValidate(value) {
    ...
    for (const [prefix, type] of typeMap) {
  ...
  ticketInput.addEventListener('input', () => {
    detectTypeAndValidate(ticketInput.value);
  });
...
if (!form) return;
  ...
  // 🔥 usa validação nativa do browser
  ...
  <div id="export-panel" class="export-panel" hidden>
    <div class="actions" style="justify-content:flex-end; margin:8px 0 0">
      <button type="button" onclick="exportByMonth()" class="btn btn-info">
        Exportar Planilha (mês do formulário)
      </button>
    </div>
  </div>
</section>
...
(() => {
  ...
  if (!form || !openBtn || !submitBtn) return;
  ...
  checkbox.addEventListener('change', () => {
    ...
    submitBtn.style.opacity = checkbox.checked ? '1' : '.8';
  });

  // ✅ ENVIO DO FORM (PONTO MAIS IMPORTANTE)
